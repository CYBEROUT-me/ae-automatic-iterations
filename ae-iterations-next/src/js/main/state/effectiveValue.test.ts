...
describe("effectiveValue", () => {
  it("returns the row's own value when sameForAll is false", () => {
    ...
    expect(effectiveValue(rowLayers, values, false, shapeRow, 0, "itr")).toEqual({ color: [1, 0, 0] });
    expect(effectiveValue(rowLayers, values, false, textRow, 0, "itr")).toEqual({ color: [0, 1, 0], font: "Arial-Bold" });
  });
  ...
  it("borrows row 0's color for a non-first shape row when sameForAll is true", () => {
    ...
    expect(effectiveValue(rows, values, true, secondShapeRow, 0, "itr")).toEqual({ color: [1, 0, 0] });
  });
  ...
  it("borrows row 0's color and font for a non-first text row when sameForAll is true", () => {
    ...
    const result = effectiveValue(rowLayers, values, true, textRow, 0, "itr");
  ...
  it("borrows row 0's actual font string for a non-first text row when row 0 is itself a text layer", () => {
    ...
    const result = effectiveValue(rows, values, true, secondTextRow, 0, "itr");
  ...
  it("never borrows for stroke rows even when sameForAll is true", () => {
    ...
    expect(effectiveValue(rowLayers, values, true, strokeRow, 0, "itr")).toEqual({ color: [0.5, 0.5, 0.5] });
  });
  ...
  it("never borrows for video rows even when sameForAll is true", () => {
    ...
    expect(effectiveValue(rowLayers, values, true, videoRow, 0, "itr")).toEqual({
      flip: true,
      bw: false,
      tint: null,
      tintAmount: 50,
  ...
  it("returns the row's own value for row 0 itself, regardless of sameForAll", () => {
    ...
    expect(effectiveValue(rowLayers, values, true, shapeRow, 0, "itr")).toEqual({ color: [1, 0, 0] });
  });
  ...
  it("falls back to the row's own value when row 0 has no value yet for that iteration", () => {
    ...
    expect(effectiveValue(rowLayers, values, true, textRow, 0, "itr")).toEqual({ color: [0, 1, 0], font: "Arial-Bold" });
  });

  it("never borrows under VAR mode, even when sameForAll is true", () => {
    const varRowLayers: RowLayer[] = [
